...
export function DashboardNavbar({ collapsed, onToggle }: DashboardNavbarProps) {
  ...
  return (
    <aside
      className={[
        "fixed inset-y-0 left-0 z-50 hidden border-r border-white/10 bg-surface-soft/90 backdrop-blur-xl md:flex md:flex-col",
    ...
    >
      <div className="border-b border-white/10 px-4 py-4">
        <div>
          <div className="mb-3 flex items-center justify-between">
            <p className={["font-brand text-xs uppercase tracking-[0.2em] text-slate-400", collapsed ? "hidden" : "block"].join(" ")}>
              VCG Tariff Analysis
            </p>
            <button
              onClick={onToggle}
              className="rounded-md border border-white/10 bg-white/5 p-2 text-slate-200 hover:bg-white/10"
              aria-label={collapsed ? "Expand sidebar" : "Collapse sidebar"}
            >
              {collapsed ? <PanelLeftOpen size={14} /> : <PanelLeftClose size={14} />}
            </button>
          </div>
          <h1 className={["font-brand font-semibold text-white", collapsed ? "text-xs text-center" : "text-lg sm:text-xl"].join(" ")}>
      ...
      <nav className="hide-scrollbar flex-1 overflow-y-auto px-3 py-4">
        {sections.map((section) => (
          <div key={section} className="mb-4 space-y-1">
            <p className={["px-2 pb-1 text-[10px] uppercase tracking-[0.2em] text-slate-500", collapsed ? "hidden" : "block"].join(" ")}>
              {section}
            </p>
            {navItems
      ...
      <div className="border-t border-white/10 p-3">
        <div className={["rounded-xl border border-white/10 bg-white/5 p-3", collapsed ? "flex justify-center" : "block"].join(" ")}>
          {collapsed ? (
            <CircleUserRound size={18} className="text-slate-200" />
          ) : (
            <>
